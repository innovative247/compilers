...
const string Usage = "Usage: set_table_locations <server/profile> [-U user] [-P pass] [-O outfile] [-MSSQL|-SYBASE]";
if (!VersionCheck.CheckForUpdates("set_table_locations", args, Usage)) return 0;
...
return InteractiveMenus.RunSetTableLocations(cmdvars, profile, executor, headlessArgs);
